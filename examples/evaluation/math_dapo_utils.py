...
def _accuracy(predictions, labels) -> float:
    ...
    for prediction, label in zip(predictions, labels):
        is_correct, _ = is_correct_minerva(prediction, str(label))
        correct += int(is_correct)
    return correct / max(len(labels), 1)


def eval_fn(predictions, labels):
    """Entry point for --custom_eval_fn: DAPO/AIME-style exact-match accuracy."""
    return {"accuracy": _accuracy(predictions, labels)}


def reward_fn(responses, labels):
    """Entry point for --custom_reward_fn: per-step exact-match reward."""
    return {"reward/accuracy": _accuracy(responses, labels)}
