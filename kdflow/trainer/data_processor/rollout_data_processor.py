...
class RolloutDataProcessor:
    ...
    def process(
        ...
        if mode == "train" and self.reward_fn is not None:
            responses = [sample["stu_responses"][0] for sample in sample_list]
            rollout_metrics.update(
                {f"rollout/{key}": value for key, value in self.reward_fn(responses, labels).items()}
            )
