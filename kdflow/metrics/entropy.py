...
def compute_entropy(student_logits, **kwargs):
    ...
    with torch.no_grad():
        if student_logits.shape[0] == 0:
            return {"distill/student_entropy": student_logits.new_zeros(())}

        # Chunk over tokens to bound peak memory; each chunk reuses a single
        # [chunk_tokens, vocab] intermediate via in-place exp/div.
        chunk_tokens = 2048
        entropy_parts = []
